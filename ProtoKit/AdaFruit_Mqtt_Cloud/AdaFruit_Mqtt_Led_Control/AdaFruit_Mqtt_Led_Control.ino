...
Adafruit_MQTT_Subscribe Led1 = Adafruit_MQTT_Subscribe(&mqtt, AIO_USERNAME "/feeds/Led_1");
Adafruit_MQTT_Subscribe Led2 = Adafruit_MQTT_Subscribe(&mqtt, AIO_USERNAME "/feeds/Led_2");
Adafruit_MQTT_Subscribe Led3 = Adafruit_MQTT_Subscribe(&mqtt, AIO_USERNAME "/feeds/Led_3");
/*************************** Sketch Code ************************************/
...
void setup() {
  ...
  Serial.println(F("ProtoKit Led Control"));
  ...
  pinMode(P.Led_2, OUTPUT);
  pinMode(P.Led_3, OUTPUT);
}
...
void loop() {
  ...
  while ((subscription = mqtt.readSubscription(5000))) {
    if (subscription == &Led1) 
    {
      ...
      int state = atoi((char *)Led1.lastread); //Convert ASCII to Integer and store in state
    ...
    if (subscription == &Led2) 
    {
      ...
      int state = atoi((char *)Led2.lastread); //Convert ASCII to Integer and store in state
    ...
    {
      Serial.print(F("Led_3: "));
      Serial.println((char *)Led3.lastread);
      
      int state = atoi((char *)Led3.lastread); //Convert ASCII to Integer and store in state
